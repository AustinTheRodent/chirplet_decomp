...
if [ $main_arg == "all" ] || [ $main_arg == "hw" ] || [ $main_arg == "sw" ] || [ $main_arg == "onlygen" ]; then
  ...
  rm -f $params_input_fname
  touch $params_input_fname

  echo $time_step > $params_input_fname
  echo $tau >> $params_input_fname
  echo $alpha1 >> $params_input_fname
  echo $f_c >> $params_input_fname
  echo $alpha2 >> $params_input_fname
  echo $phi >> $params_input_fname
  echo $beta >> $params_input_fname

  $script_dir/c_code/bin/txt_to_bin \
    -i $params_input_fname \
    -o ${params_input_fname%txt*}bin \
    -t float
...
if [ $main_arg == "all" ] || [ $main_arg == "sw" ];then
  ...
  cp $params_input_fname $script_dir/sw_output/output.txt
...
-g/tb_${sim_name}/G_CHIRPLET_SAMPS=$num_samps \
-g/tb_${sim_name}/G_PARAMS_INPUT_FNAME=$params_input_fname \
